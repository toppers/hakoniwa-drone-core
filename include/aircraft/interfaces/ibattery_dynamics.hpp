...
protected:
    void *vendor_model;
    void *context;
    config::BatteryModelParameters params;
...
public:
    ...
    virtual void add_device(ICurrentDynamics& device)
    {
        ...
        if (entry == nullptr) {
            throw std::runtime_error("Failed to allocate memory for DischargeDynamicsType");
        }
        entry->device = &device;
    ...
    virtual void set_params(const config::BatteryModelParameters &p) = 0;
