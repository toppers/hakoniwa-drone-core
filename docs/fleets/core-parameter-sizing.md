...
一般向け配布バイナリの契約上限は200機体である。200機体を超える値は、箱庭ドローンPROライセンスと非公開ソースへのアクセス権を持つユーザーが、Foundationを含む `research-512` プロファイルを再ビルドした場合に限り使用できる。具体的な入口は `config/build/hakoniwa-build-512.yaml` と [再ビルド・クリーン再起動手順](rebuild-and-restart.md)を参照する。

スケーラビリティ比較では、ビルド条件を揃えるため、200機体以下を含む全測定点で同じ `research-512` バイナリを使用する。
