...
箱庭 service 制御を使う場合は、fleet file に `serviceConfigPath` を持たせ、type 定義側で service mode を指定する。

- RC 操作:
  - `controller.serviceMode = "rc"`
- 新しい箱庭 RPC service:
  - `controller.serviceMode = "api"`
  - `controller.apiServiceMode = "rpc"`
- 従来の PDU ベース API:
  - `controller.serviceMode = "api"`
  - `controller.apiServiceMode = "legacy-api"`

後方互換:

- 既存の `controller.serviceMode = "rpc"` は、
  - `controller.serviceMode = "api"`
  - `controller.apiServiceMode = "rpc"`
  と同義として扱う
