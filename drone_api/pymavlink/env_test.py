...
def run_mission(client: MavlinkMultirotorClient) -> bool:
    ...
    if client.takeoff(1.0, timeout_sec=6000):
        ...
        if client.moveToPosition(-500.0, 0.0, 20.0, 20.0, yaw_deg=180, timeout_sec=6000):
            print("Move 1 successful.")
            time.sleep(2)

            if client.moveToPosition(-500.0, -500.0, 20.0, 20.0, yaw_deg=-90, timeout_sec=6000):
                print("Move 2 successful.")
                time.sleep(2)

                if client.moveToPosition(0.0, 0.0, 20.0, 20.0, yaw_deg=45, timeout_sec=6000):
