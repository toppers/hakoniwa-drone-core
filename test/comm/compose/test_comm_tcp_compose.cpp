...
class CommComposeIOTest : public ::testing::Test {
protected:
    ...
    void SetUp() override
    {
        ...
        io_ = client->client_open(nullptr, &dst);
...
TEST_F(CommComposeIOTest, SendValidData)
{
...
TEST_F(CommComposeIOTest, SendNullDataPointer)
{
...
TEST_F(CommComposeIOTest, SendNegativeLength)
{
...
TEST_F(CommComposeIOTest, SendNullSendLenPointer)
{
    const char* msg = "Hello TCP Server";
    const int   datalen = static_cast<int>(strlen(msg));
...
TEST_F(CommComposeIOTest, RecvSuccess)
{
...
TEST_F(CommComposeIOTest, RecvNullDataPointer)
{
    const char* msg = "Ping";
    int         sent = 0;
...
TEST_F(CommComposeIOTest, RecvNegativeLength)
{
    const char* msg = "Ping";
    int         sent = 0;
    io_->send(msg, static_cast<int>(strlen(msg)), &sent);
...
TEST_F(CommComposeIOTest, RecvNullRecvLenPointer)
{
    const char* msg = "Hello Server";
    int         sent = 0;
    io_->send(msg, static_cast<int>(strlen(msg)), &sent);
