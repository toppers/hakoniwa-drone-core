...
void runServer(int portno) {
    ...
    std::cout << "[TCP] Server is starting open..." << std::endl;
    ...
    std::cout << "[TCP] Server opened connection." << std::endl;
    ...
    if (received) {
        std::cout << "[TCP] Server received data: " << std::string(buffer, receivedLen) << std::endl;
    }
...
protected:
    static inline int basePort = 19080;
    int portno;
    void SetUp() override {
        portno = basePort++;
